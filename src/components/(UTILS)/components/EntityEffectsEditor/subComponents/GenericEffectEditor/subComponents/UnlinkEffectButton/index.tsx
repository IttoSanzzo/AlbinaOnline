...
const UnlinkButton = newStyledElement.button(styles.unlinkButton);
...
interface UnlinkEffectButtonProps {
	safetyText: string;
	linkId: Guid;
	pathname: string;
	revalidatePath?: string;
}
export function UnlinkEffectButton({
	safetyText,
	linkId,
	pathname,
	revalidatePath,
}: UnlinkEffectButtonProps) {
	...
	async function onSubmit(data: FormData) {
		...
		const response = await authenticatedFetchAsync(`/effect-links/${linkId}`, {
			method: "DELETE",
		});
		if (!response.ok) {
			toast.error("Unlinking error", { id: toastId });
			return;
		}
		toast.success("Unlinked", { id: toastId });
		setOpenState(false);
	...
	return (
		<AlertDialog.Root
			...
			<AlertDialog.Trigger asChild>
				<UnlinkButton>Unlink Effect</UnlinkButton>
			</AlertDialog.Trigger>
			<AlertDialog.Portal>
				<AlertDialog.Overlay>
					<AlertDialog.Content>
						<AlertDialog.Title>Unlink</AlertDialog.Title>
						<HookedForm.Form
							form={form}
							onSubmit={onSubmit}>
