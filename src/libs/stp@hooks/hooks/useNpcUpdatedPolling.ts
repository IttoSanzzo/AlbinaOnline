...
export function useNpcUpdatedPolling(npcId: string) {
	const lastUpdatedAt = useRef<string | null>(null);

	async function loadUpdatedAt(): Promise<string | null> {
		const response = await authenticatedFetchAsync(
			getAlbinaApiFullAddress(`/npcs/${npcId}/updated-at`),
			{ method: "GET" },
		);
		...
		const tick = async () => {
			...
			try {
				...
				if (
					...
					const success = await eventBus.emitAsync(`/npcs/${npcId}/updated`, {
						updatedAt,
					});
					if (!isMounted) return;
					if (success) {
